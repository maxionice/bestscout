# BestScout documentation

- [English user guide](user-guide.en.md)
- [Deutsches Benutzerhandbuch](user-guide.de.md)
- [Roadmap](roadmap.md)
- [Feature-parity specification](feature-parity.md)
- [Current resumable handoff](handoff.md)
- [Linux packaging and releases](release/linux-packaging.md)
- [Security policy](../SECURITY.md)

Architecture, compatibility and acceptance records live in their respective
subdirectories. The user guides describe available controls; acceptance records
remain authoritative for whether a live FM capability is enabled.
